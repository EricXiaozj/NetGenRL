...
class Generator(nn.Module):
    def __init__(self, label_dim, seq_dim, max_seq_len, x_list, device):
        ...
        self.emb = nn.Embedding(seq_dim, 128)
        ...
        self.combine_fc = nn.Sequential(
            nn.Linear(128+64+64,self.hidden_dim),
            nn.ReLU(True)
        )
    ...
    def forward(self, label, length, x):
        ...
        combined = torch.cat([emb, label_out, length_out], dim=1)
    ...
    def step(self, label, length, x, h, c):
        ...
        output = F.output_layer(hidden_output.view(-1, self.hidden_dim),dim = 1)
    ...
    def sample(self, batch_size, seq_len, label, length, x=None):
        ...
        if flag:
            ...
            for i in range(given_len):
                preds, h, c = self.step(label, length, lis[i], h, c)
            ...
            for i in range(given_len, seq_len):
                samples.append(sam)
                output, h, c = self.step(label, length, x, h, c)
                count = 0
                for x_len in self.x_list:
                    pred = preds[:,count:count + x_len]
                    sam.append(pred.multinomial(1))
...
class CustomDataset(Dataset):
    def __init__(self, json_file, class_mapping, max_seq_len, set_file, transform=None):
        """

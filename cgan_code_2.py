...
# LABEL_COUNT_DICT = {'facebook': 0.0028654915429916486, 'skype': 0.0038945699772322466, 'aim': 0.22332924713191163, 'email': 0.023981664121547557, 'voipbuster': 0.007776426450730328, 'hangouts': 0.0006573945274787207, 'youtube': 0.03355181177568625, 'sftp': 0.25523342529361326, 'icq': 0.23053341639423136, 'ftps': 0.057172287265769374, 'vimeo': 0.05254805814868509, 'spotify': 0.05216449568044651, 'netflix': 0.041309456116885385, 'bittorrent': 0.014982255572790718}
# LABEL_DICT = {'facebook': 0, 'skype': 1, 'email': 2, 'voipbuster': 3, 'hangouts': 4, 'youtube': 5, 'ftps': 6, 'vimeo': 7, 'spotify': 8, 'netflix': 9, 'bittorrent': 10}
# LABEL_COUNT_DICT = {'facebook': 0.009850302566802523, 'skype': 0.013387822671174656, 'email': 0.08243843826042112, 'voipbuster': 0.02673194189510935, 'hangouts': 0.00225983392526957, 'youtube': 0.11533640658030749, 'ftps': 0.19653323681284396, 'vimeo': 0.1806371661882757, 'spotify': 0.1793186467270474, 'netflix': 0.1420037838243092, 'bittorrent': 0.05150242054843919}
...
class Generator(nn.Module):
    ...
    def forward(self, label, noise, length):
        ...
        # label_noise = torch.randn_like(label) * 0.1         # 添加高斯噪声
        # smooth_label = (label + label_noise).clamp(0, 1)
        # length_noise = torch.randn_like(length_one_hot) * 0.1         # 添加高斯噪声
        # smooth_length = (length_one_hot + length_noise).clamp(0, 1)
        ...
        combined = torch.cat([label_out, length_out, noise_out], dim=1)
...
def get_non_matching_labels_one_hot(labels_indices, num_classes, device):
    # 获取当前批次的标签
    current_labels = torch.argmax(labels_indices.clone(),dim=1)
...
def diversity_loss_variance_multidim(sequences, lengths, max_seq_len, diversity_weight=1.0):
    """
    计算生成序列的多样性损失，基于每个时间点的多维值方差。
    
    参数:
    - sequences (torch.Tensor): 填充后的序列张量，形状为 (batch_size, max_length, feature_dim)。
    - lengths (torch.Tensor): 每个序列的实际长度，形状为 (batch_size,)。
    - diversity_weight (float): 多样性损失的权重。
    
    返回:
    - loss (torch.Tensor): 多样性损失。
    """
    # batch_size, max_length, feature_dim = sequences.shape
    ...
    for t in range(max_seq_len):
        # 提取时间点 t 的所有序列的值
        time_step_values = sequences[:, t, :]  # 形状为 (batch_size, feature_dim)
        
        # 找到该时间点有效的样本（根据 lengths 判断）
        valid_mask = (lengths > t)  # 如果序列长度 > t，则该时间点有效
        valid_values = time_step_values[valid_mask]  # 提取有效样本，形状为 (num_valid_samples, feature_dim)
        
        if valid_values.size(0) > 1:  # 至少需要两个样本才能计算方差
            # 计算该时间点每个维度的方差
            time_step_variance = valid_values.var(dim=0, unbiased=False)  # 形状为 (feature_dim,)
            variances.append(time_step_variance)
    
    # 如果没有有效的方差，返回 0
    if not variances:
        return torch.tensor(0.0, requires_grad=True, device=sequences.device)
    
    # 将所有时间点的方差求均值（按 feature_dim 聚合）
    mean_variances = torch.stack(variances).mean()  # 标量
    diversity_loss = mean_variances  # 最大化方差（取负号）
    
    return torch.log(diversity_loss)


# %%
def train(conditionGAN, dataloader, epochs, device, clip_value=0.01, alpha = 0.5):
    # 使用Wasserstein GAN损失
    ...
    for epoch in range(epochs):
        for i, (seqs, labels, lengths, weights) in enumerate(dataloader):
            ...
            for _ in range(N_CRITIC):
                ...
                # wrong_labels = get_non_matching_labels_one_hot(labels, len(LABEL_DICT), device)
                # wrong_validity = conditionGAN.discriminator(wrong_labels, seqs, lengths)
                
                d_loss_real_wo_w = -torch.mean(real_validity)
                d_loss_fake_wo_w = torch.mean(fake_validity)
                # d_loss_wrong_wo_w = torch.mean(wrong_validity)
                
                # 权重
                # real_validity *= weights
                # fake_validity *= weights
                # wrong_validity *= weights
                
                d_loss = -torch.mean(real_validity) + torch.mean(fake_validity)
            
                d_loss_real = -torch.mean(real_validity)
                d_loss_fake = torch.mean(fake_validity)
                # d_loss_wrong = torch.mean(wrong_validity)
            ...
            validity = conditionGAN.discriminator(labels, fake_seqs.detach(),lengths)
            
            g_loss_wo_w = -torch.mean(validity)
            
            # validity *= weights
            
            # diversity_loss = - diversity_loss_variance_multidim(fake_seqs.detach(), lengths, MAX_SEQ_LEN, diversity_weight=1.0)
            # diversity_loss_real = diversity_loss_variance_multidim(seqs, lengths, MAX_SEQ_LEN, diversity_weight=1.0)
            
            # diversity_loss = - diversity_loss_fake
        ...
        # 每个epoch结束时保存模型
        print(f"Epoch [{epoch+1}/{epochs}], D Loss: {d_loss.item()} ({d_loss_real.item()}/{d_loss_real_wo_w.item()}, {d_loss_fake.item()}/{d_loss_fake_wo_w.item()}), G Loss: {g_loss.item()}/{g_loss_wo_w.item()}).")
        # print(f"Epoch [{epoch+1}/{epochs}], Real: {rp}, Fake: {fp}.")

        torch.save(conditionGAN.generator.state_dict(), './save_all/generator.pth')
        torch.save(conditionGAN.discriminator.state_dict(), './save_all/discriminator.pth')

        if (epoch + 1) % 50 == 0:
            torch.save(conditionGAN.generator.state_dict(), f'./save_all/generator_{epoch+1}.pth')
            torch.save(conditionGAN.discriminator.state_dict(), f'./save_all/discriminator_{epoch+1}.pth')
...
if __name__ == '__main__':
    ...
    dataset = CustomDataset(json_file='./vpn_data_small.json', class_mapping=LABEL_DICT,max_seq_len=max_seq_len,bins_file= './bins_small.json')
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)


    # checkpoint_g = torch.load('./save_all/generator.pth')  # 加载保存的权重字典
    # cg.generator.load_state_dict(checkpoint_g) 

    # checkpoint_d = torch.load('./save_all/discriminator.pth')  # 加载保存的权重字典
